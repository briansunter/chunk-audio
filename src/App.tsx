...
import type { AudioFile, CutPoint, Segment, EditorMode, ExportOptions } from './types';
...
export default function App() {
  ...
  // Compute chunk-mode cut points and segment lines
  const chunkCutPoints = useMemo((): CutPoint[] => {
    if (!audioFile) return [];
    const points: CutPoint[] = [];
    for (let t = chunkDuration; t < audioFile.duration; t += chunkDuration) {
      points.push({ id: `chunk-${t}`, time: t });
    }
    return points;
  }, [audioFile, chunkDuration]);
  ...
  const effectiveSegments = useMemo((): Segment[] => {
    if (!audioFile) return [];
    if (mode === 'chunk') {
      return deriveSegments(chunkCutPoints, audioFile.duration);
    }
    return editor.segments;
  }, [mode, audioFile, chunkCutPoints, editor.segments]);
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        case ' ':
          e.preventDefault();
          editor.playbackState.isPlaying ? editor.pause() : editor.play();
          break;
        case 'v':
          editor.setTool('select');
          break;
        case 'b':
          editor.setTool('blade');
          break;
        case 'Delete':
        case 'Backspace':
          if (mode === 'manual' && editor.cutPoints.length > 0) {
            editor.removeCutPoint(editor.cutPoints[editor.cutPoints.length - 1].id);
          }
          break;
      }
    };
  ...
  const handleExport = useCallback(async (options: ExportOptions) => {
    ...
    try {
      ...
      if (options.mode === 'chunk') {
        files = await chunkByTime(ffmpeg, inputName, options.chunkDuration, options.outputFormat);
      } else {
        const enabledSegments = editor.segments.filter(s => s.enabled);
        files = await extractSegments(ffmpeg, inputName, enabledSegments, options.outputFormat);
      }
      ...
      const toClean = allFiles.filter(f => f !== inputName && !f.startsWith('.'));
      ffmpeg.cleanup(...toClean);
    }
  }, [audioFile, ffmpeg, editor.segments]);
          ...
          <section className="landing-shell">
            <div className="landing-copy">
              ...
              <p className="landing-description">
                Drop a file, place cuts with the blade tool, rearrange segments, then export as ZIP. Everything runs locally via WebAssembly.
              </p>
            </div>
            ...
            <div className="landing-features">
              <article className="feature-card">
                ...
                <p className="feature-copy">Click to place cut points on the waveform. Drag to reposition, right-click to remove.</p>
              </article>
              <article className="feature-card">
                <p className="feature-label">Private</p>
                <p className="feature-copy">All processing in-browser via WebAssembly. Your files never leave your machine.</p>
              </article>
              <article className="feature-card">
                <p className="feature-label">Export</p>
                <p className="feature-copy">Chunk by time or export manual cuts as MP3, WAV, or OGG in a single ZIP.</p>
              </article>
            </div>
          </section>
        ) : (
                ...
                <div className="editor-workspace flex-1 min-h-0">
                  <div className="flex items-center justify-between">
                    <div className="meta-strip">
                      ...
                      <span className="meta-pill">{formatBytes(audioFile.file.size)}</span>
                      <span className="meta-pill">{audioFile.audioBuffer.sampleRate}Hz</span>
                    </div>
                    <Toolbar
                ...
                {effectiveSegments.length > 1 && (
                  <div className="bg-card border border-border rounded-lg flex-1">
                    <SegmentTrack
                      ...
                      onReorder={mode === 'manual' ? editor.reorderSegments : undefined}
                      onToggleEnabled={mode === 'manual' ? editor.toggleSegmentEnabled : undefined}
                    />
                  </div>
                )}
...
function formatBytes(bytes: number): string {
  ...
  return `${(bytes / Math.pow(step, i)).toFixed(i === 0 ? 0 : 1)} ${units[i]}`;
}
